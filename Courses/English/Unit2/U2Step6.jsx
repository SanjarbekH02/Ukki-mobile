...
export default function ListenAndChooseGrid({ next }) {
  ...
  const playPrompt = async (index) => {
    ...
    try {
      ...
      setIsPlaying(false);
      Alert.alert("Xato", "Audio faylni yuklashda xatolik yuz berdi.");
    }
  };
  ...
  const checkAnswers = () => {
    ...
    if (correct === total) {
      setAllCorrect(true);
      Alert.alert("Tabriklaymiz!", "Siz barcha javoblarni to‘g‘ri topdingiz!");
    } else {
      setAllCorrect(false);
      Alert.alert(
        "Tekshirish",
